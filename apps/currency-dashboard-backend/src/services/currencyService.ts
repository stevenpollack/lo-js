...
export class CurrencyService {
  ...
  public async getRates(
    ...
    try {
      ...
      const filteredRates = targetCurrencies.reduce(
        (acc: Record<string, number>, currency: string) => {
          if (rates[currency] === undefined) {
            this.logger.warn(
              `Rate for ${currency} not found in response for base ${baseCurrency}`,
            );
          } else {
      ...
      this.logger.debug(
        `Returning ${Object.keys(filteredRates).length} rates for ${baseCurrency}`,
      );
      return filteredRates;
    } catch (error) {
      this.logger.error(
        `Error fetching exchange rates for ${baseCurrency}`,
        error,
      );
  ...
  public async getAvailableCurrencies(): Promise<string[]> {
    ...
    try {
      ...
      if (cachedCurrencies) {
        this.logger.debug(
          `Using ${cachedCurrencies.length} cached available currencies`,
        );
